...
def affine_forward(x, w, b):
    ...
    D = np.prod(x.shape[1:]) # Calculate the dimension D by multiplying all other dimensions
    x_reshaped = x.reshape(N, D) # Reshape x to (N, D)
    out = np.dot(x_reshaped, w) + b # Perform the affine transformation
...
def affine_backward(dout, cache):
    ...
    # Calculate gradients
    dx_reshaped = np.dot(dout, w.T) # Gradient with respect to reshaped x, shape (N, D)
    dw = np.dot(x_reshaped.T, dout) # Gradient with respect to w, shape (D, M)
...
def relu_backward(dout, cache):
    ...
    # Multiply the upstream gradient by the mask
    dx = dout * mask
        ...
        for f in range(F):  # For each filter
            for i in range(H_out):
                for j in range(W_out):
                    # Get the current receptive field
                    h_start = i * stride
                    h_end = h_start + HH
                    w_start = j * stride
                    w_end = w_start + WW
        ...
        for c in range(C):
            for i in range(H_out):
                for j in range(W_out):
                    ...
                    # Only the max value gets gradient
        ...
        for c in range(C):
            for i in range(H_out):
                for j in range(W_out):
                    ...
                    # Gradient is distributed equally to all elements in the window
                    dx[n, c, h_start:h_end, w_start:w_end] += dout[n, c, i, j] / (pool_height * pool_width)
...
def spatial_batchnorm_forward(x, gamma, beta, bn_param):
    ...
    # Reshape x to (N*H*W, C) for batchnorm
    x_reshaped = x.transpose(0, 2, 3, 1).reshape(-1, C)
    
    # Initialize running_mean and running_var if not provided
    if 'running_mean' not in bn_param:
        bn_param['running_mean'] = np.zeros(C, dtype=x.dtype)
    if 'running_var' not in bn_param:
        bn_param['running_var'] = np.zeros(C, dtype=x.dtype)
...
def spatial_layernorm_forward(x, gamma, beta, ln_param):
    ...
    # Reshape gamma and beta to (H*W*C,)
...
def spatial_layernorm_backward(dout, cache):
    ...
    # Reshape dout to (N, H*W*C)
    dout_reshaped = dout.reshape(N, -1)
    
    # Perform layernorm backward
    dx_reshaped, dgamma_reshaped, dbeta_reshaped = layernorm_backward(dout_reshaped, cache)
    
    # Reshape back to original dimensions
    dx = dx_reshaped.reshape(N, C, H, W)
    dgamma = dgamma_reshaped.reshape(C, H, W)
    dbeta = dbeta_reshaped.reshape(C, H, W)
...
def instancenorm_forward(x, gamma, beta, in_param):
    ...
    if 'mode' not in in_param:
        in_param['mode'] = 'train'  # Default to train mode
    if 'eps' not in in_param:
        in_param['eps'] = 1e-5      # Default epsilon
    
    # Reshape x to (N*H*W, C) for normalization across spatial dimensions
    x_reshaped = x.transpose(0, 2, 3, 1).reshape(-1, C)
    
    # Calculate mean and variance for each channel across spatial dimensions
    mean = x_reshaped.mean(axis=0, keepdims=True)
    var = x_reshaped.var(axis=0, keepdims=True)
    
    # Normalize
    x_normalized = (x_reshaped - mean) / np.sqrt(var + in_param['eps'])
    
    # Reshape back to (N, H, W, C)
    x_normalized = x_normalized.reshape(N, H, W, C)
    
    # Apply instance-specific gamma and beta
    # gamma shape (N, C), beta shape (N, C)
    out = gamma.reshape(N, 1, 1, C) * x_normalized + beta.reshape(N, 1, 1, C)
    
    # Transpose back to (N, C, H, W)
    out = out.transpose(0, 3, 1, 2)
    
    # Store values needed for backward pass
    cache = (x, x_normalized, mean, var, gamma, beta, in_param['eps'])
...
def instancenorm_backward(dout, cache):
    ...
    # Gradient for beta (sum over H,W)
    dbeta = np.sum(dout, axis=(1, 2))
    
    # Gradient for gamma (sum over H,W)
    dgamma = np.sum(dout * x_normalized, axis=(1, 2))
    
    # Reshape for batch norm style backward pass
    x_normalized_flat = x_normalized.reshape(-1, C)
    dout_flat = (dout * gamma.reshape(N, 1, 1, C)).reshape(-1, C)
    
    # Intermediate gradients
    dx_normalized_flat = dout_flat
    
    # Variance gradient
    dvar = np.sum(dx_normalized_flat * (x_normalized_flat * (-0.5) * (var + eps) ** (-1.5)), axis=0)
    
    # Mean gradient
    N_total = N * H * W
    dmean = np.sum(dx_normalized_flat * (-1 / np.sqrt(var + eps)), axis=0)
    dmean += dvar * np.sum(-2 * (x_normalized_flat * np.sqrt(var + eps) - mean), axis=0) / N_total
    
    # Input gradient
    dx_flat = dx_normalized_flat / np.sqrt(var + eps)
    dx_flat += dvar * 2 * (x_normalized_flat * np.sqrt(var + eps) - mean) / N_total
    dx_flat += dmean / N_total
